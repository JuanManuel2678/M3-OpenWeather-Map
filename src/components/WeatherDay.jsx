  ...
  return (
      ...
      {forecast && forecast.slice(0, 5).map(item => (
        ...
        <figure className="h-[62px] w-full ">
          < img src={`/${item?.weather[0]?.icon}.png`} alt="icon weather day" className="h-full w-full object-contain"/>
        </figure>
        <div className="flex justify-between">
          <div className="text-[16px] font-medium text-[#E7E7EB]">{Math.trunc(item?.main?.temp_max)} °C</div>
          <div className="text-[16px] font-medium text-[#A09FB1]">{Math.trunc(item?.main?.temp_min)} °C</div>
        </div>
        </li>
      )) }
